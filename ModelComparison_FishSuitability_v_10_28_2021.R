...
###################################
  ...
  #load("~/DisMAP project/Location, Location, Location/Location Workshop/saved_models_10_28_21.RData")  #full models
  
  dat_fcast$log_abundance <- log(dat_fcast$abundance)
  dat_all<-rbind(dat_hist, dat_fcast)
  dat_fcast_a<-dat_fcast[dat_fcast$year>=2011,]
  dat_fcast_1<-dat_fcast_a[dat_fcast_a$year<2040,]
  dat_fcast_b<-dat_fcast[dat_fcast$year>=2040,]
  dat_fcast_2<-dat_fcast_b[dat_fcast_b$year<2070,]
  dat_fcast_3<-dat_fcast[dat_fcast$year>=2070,]
  
  ### GO TO DEV EXPLAINED FILE TO RUN that infor!!!!!
  
  #### This section runs RMSE for abundance, COG, and correlation, and spatial RMSE####

    #GO TO Alt_timeseriesPlots_v4_23_21.R for paper quality figure plots
    #GO TO Within_model_uncertainty.R to plot the model uncertainty for the GAMs fit to full set of Env conditions 
 
  ##RMSE - Abundance
  source("~/DisMAP project/Location, Location, Location/Location Workshop/RMSE_GBonly_FuturePeriods.R") 
  rmse_results<-sdm_rmse(dat_hist=dat_hist, dat_fcast_1=dat_fcast_1, dat_fcast_2=dat_fcast_2,
                         dat_fcast_3=dat_fcast_3)
  # rmse_results
  rmse_all<-data.frame(rmse_results$rmse_abund_all)
  rmse_year<-data.frame(rmse_results$rmse_abund_allh_y)
  rmse_agg<-data.frame(rmse_results$rmse_abund_annual)
  write.csv(rmse_all, 'RMSE_all_time_periods_11_8_21_had_Stephs.csv',row.names = FALSE)
  ...
  cor_hist_y<-data.frame(Cor_sdm$cor_abund_hist_year)
  cor_fcast_y<-data.frame(Cor_sdm$cor_abund_all_year)
  cor_all_year<-rbind(cor_hist_y, cor_fcast_y)
  write.csv(cor_all_year, 'COR_all_yearly.csv',row.names = FALSE)
  ...
  RMSE_cog
  COG_hist<-data.frame(RMSE_cog$cog_lat_hist)
  COG_fcast<-data.frame(RMSE_cog$cog_lat_fcast)
  COG_all<-rbind(COG_hist, COG_fcast)
  write.csv(COG_all, 'cog_all.csv', row.names=FALSE)
  
  # ##Species Distribution Maps 
  # source("~/DisMAP project/Location, Location, Location/Location Workshop/SDM_plots_v2_14_2021.R") 
  # SDM_maps(dat_hist=dat_hist, dat_fcast=dat_fcast, YEARS=c("2000", "2040", "2100"))
  # 
  #spatial error
  ...
  # SE_hist<-data.frame(SRMSE$spatial_err_all_hist)
  # SE_fcast<-data.frame(SRMSE$spatial_err_all_fcast)
  # write.csv(SE_hist, 'SE_hist.csv', row.names=FALSE)
  # write.csv(SE_fcast, 'SE_fcast.csv', row.names=FALSE)